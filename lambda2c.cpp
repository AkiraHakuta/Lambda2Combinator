...
string eval(string data, bool trace, int step){ 
    bool changed = false;
    ...
    if (changed)
        return eval(result_str, trace, step);        
    else
        return result_str;
}
...
{   
    ...
    for (int i = 0; i < e->comm.size(); i++){
        ...
        result_str =  eval(e->comm[i], trace, step);
        if (!trace)
